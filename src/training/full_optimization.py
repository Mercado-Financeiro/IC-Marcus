...
from src.utils.memory_utils import (
    memory_cleanup, mlflow_run_context, log_memory_usage,
    safe_delete, optimize_memory_settings, MemoryTracker
)
...
def run_xgboost_optimization():
    ...
    with memory_cleanup(track=True):
    ...
    log_memory_usage("After XGB Optimization")
    ...
    # 12. Log to MLflow with proper cleanup
    with mlflow_run_context(run_name="xgboost_production_100trials") as run:
        # Log parameters
    ...
    safe_delete(df, X, y, X_train, X_test, y_train, y_test)
    safe_delete(xgb_opt, study, model)
    gc.collect()
    
    # Log final memory usage
    memory_summary = tracker.stop()
    log_memory_usage("XGB End")
    print(f"\n📊 Memory Usage Summary:")
    print(f"  Peak: {memory_summary.get('peak_memory_mb', 0):.1f} MB")
    print(f"  Delta: {memory_summary.get('total_delta_mb', 0):.1f} MB")
